...
from svi.config import ZonesConfig, view_key

ZONE_LABELS = ("great", "fair", "poor")
...
def score_view(
    ...
    *,
    ...
    ranked = (
        scored.dropna(subset=["cost_per_fps"]).sort_values("cost_per_fps").reset_index(drop=True)
    )
    ranked.insert(0, "rank", range(1, len(ranked) + 1))

    zmode = zones.mode_for(view)
    great_max, fair_max = zone_thresholds(ranked["cost_per_fps"], zmode, zones)
    ranked["zone"] = ranked["cost_per_fps"].map(lambda c: assign_zone(c, great_max, fair_max))
    meta = {"view": view, "mode": zmode, "great_max": great_max, "fair_max": fair_max}
    return ranked, meta


def score_all_views(
    effective: pd.DataFrame, prices: pd.DataFrame, zones: ZonesConfig, views: list[str]
) -> tuple[dict[str, pd.DataFrame], dict[str, dict]]:
    rankings: dict[str, pd.DataFrame] = {}
    meta: dict[str, dict] = {}
    for view in views:
        resolution, mode = view.rsplit("_", 1)
        ranked, m = score_view(effective, prices, zones, resolution=resolution, mode=mode)
        if ranked.empty:
            continue
